...
BOOL_FIELDS = ["has_garden","has_pool","has_balcony"]
NUM_FIELDS  = ["price","area_sqm","land_area_sqm","bedrooms","bathrooms",
               "floor_number","total_floors","garden_area_sqm",
               "down_payment","installment_years","installment_amount"]

FIELD_ORDER = [
    ("property_type","transaction_type","currency","price","area_sqm","land_area_sqm"),
    ("bedrooms","bathrooms","floor_number","total_floors","furnished","finishing"),
    ("has_garden","garden_area_sqm","has_pool","has_balcony","city","district","compound_name"),
    ("landmark_proximity","view","reference_id","phone_numbers"),
    ("down_payment","installment_years","installment_amount"),
]

LABEL_MAP = {
    "property_type":"Property Type","transaction_type":"Transaction",
    "price":"Price","currency":"Currency","area_sqm":"Area (sqm)",
    "land_area_sqm":"Land Area (sqm)","bedrooms":"Bedrooms","bathrooms":"Bathrooms",
    "floor_number":"Floor #","total_floors":"Total Floors","furnished":"Furnished",
    "finishing":"Finishing","has_garden":"Garden","garden_area_sqm":"Garden sqm",
    "has_pool":"Pool","has_balcony":"Balcony","city":"City","district":"District",
    "compound_name":"Compound","landmark_proximity":"Landmark",
    "down_payment":"Down Payment","installment_years":"Install. Years",
    "installment_amount":"Install. Amount","phone_numbers":"Phones",
    "reference_id":"Ref ID","view":"View","ad_snippet":"Ad Snippet",
    "original_message":"Original Msg","sender":"Sender","ad_date":"Date/Time",
}
...
class App(tk.Tk):
    ...
    def _build_record_view(self):
        ...
        # ── Snippet card ──────────────────────────────────────────────────────
        sc = tk.Frame(self.content_frame, bg=SNIPPET_BG,
                      highlightbackground=ACCENT, highlightthickness=1)
        sc.pack(fill="x", padx=PX, pady=(16, 4))

        tk.Label(sc, text="AD SNIPPET", bg=SNIPPET_BG, fg=ACCENT2,
                 font=("Segoe UI", 8, "bold"), anchor="w"
                 ).pack(fill="x", padx=12, pady=(10, 2))

        self.snippet_text = tk.Text(
            sc, bg=SNIPPET_BG, fg=TEXT,
            font=("Segoe UI", 11), relief="flat", wrap="word",
            bd=0, height=12, insertbackground=TEXT,
        ...
        self.meta_btn = tk.Button(
            self.meta_outer,
            text="▼  Show sender / date / original message",
        ...
        self.meta_card = tk.Frame(self.meta_outer, bg=CARD,
                                  highlightbackground=BORDER, highlightthickness=1)
        for key in ["sender", "ad_date", "original_message"]:
            row = tk.Frame(self.meta_card, bg=CARD)
            row.pack(fill="x", padx=12, pady=4)
            tk.Label(row, text=LABEL_MAP.get(key, key) + ":",
                     bg=CARD, fg=TEXT_DIM, font=FONT_SMALL,
                     width=16, anchor="w").pack(side="left")
            val = str(self.current.get(key) or "")
            if key == "original_message":
                t = tk.Text(row, bg=ENTRY_BG, fg=TEXT_DIM, font=FONT_SMALL,
                            relief="flat", height=5, wrap="word", bd=4)
                t.insert("1.0", val)
                t.config(state="disabled")
                t.pack(side="left", fill="x", expand=True)
            else:
                tk.Label(row, text=val, bg=CARD, fg=TEXT_DIM,
        ...
        if self.meta_visible:
            ...
            self.meta_btn.config(
                text="▲  Hide sender / date / original message")
    ...
    def _fetch_records(self):
        ...
        cur.execute(
            f"SELECT * FROM {self.table_name} ORDER BY ad_date ASC, id ASC")
    ...
    def _build_sidebar_item(self, idx, rec):
        ...
        try:
            price_str = f"{int(price):,} {curr}".strip() if price else ""
        except Exception:
    ...
    def _save_current(self):
        ...
        try:
            self.conn.execute(
                f"UPDATE {self.table_name} SET {cols} WHERE id=?", vals)
            self.conn.commit()
