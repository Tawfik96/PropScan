...
#Property type, price, currency, bedrooms, compound_name, transaction type, ad_snippet
...
class ListingExtraction(BaseModel):
    ...
    currency: Optional[Literal["EGP", "USD", "EUR", "other"]] = Field(
        default=None,
    ...
    ad_index: int = Field(
        description="The AD number from the prompt this listing was extracted from (e.g. 1 for '--- AD 1 ---').",
    )
...
Extract ONLY these fields: property_type, transaction_type, price, currency, bedrooms, compound_name, ad_snippet, ad_index.
"ad_index" is the integer from the AD header (e.g. set 2 for "--- AD 2 ---"). If one AD block contains multiple listings, all share the same ad_index.
...
2. NO PROPERTY TYPE INFERENCE: If the specific property type word (e.g., "villa", "apartment") is not in the text, you MUST set "property_type" to null.
3. NO GEOGRAPHICAL ASSUMPTIONS: Do not use external knowledge. If the text says "Badya PalmHills", map it to "compound_name" only.
4. If a single AD block contains multiple distinct listings, return one JSON object per listing.
5. Default currency is EGP unless stated otherwise.
6. Prices with a period as thousands separator (e.g. "35.000" in a rental context) mean 35,000.
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            if DEBUG_MODEL:
                for part in response.candidates[0].content.parts:
                    if getattr(part, "thought", False):
                        ...
                        print(f"\n── Response {'─'*50}\n{part.text}\n{'─'*60}")
                return [], timing, None  # halt — do not parse or insert
            ...
            if isinstance(parsed, ListingExtraction):
                parsed = [parsed]
            return [p.model_dump() for p in parsed], timing, response
...
def insert_listing(
    ...
    conn.execute(
        ...
        INSERT INTO listings (
            ...
            bedrooms, compound_name, ad_snippet,
...
def run_pipeline(chat_file: str, days: int = 2):
    ...
    logger = open_analysis_log(run_ts, GEMINI_MODEL, total_batches, days, chat_file)
    ...
    for batch_num, batch_obj in enumerate(batches, 1):
        ...
        t_batch = _ts()
        results, bt, api_response = call_gemini(batch)   # ← unpacks 3 values now
...
if __name__ == "__main__":
    ...
    chat_file = "Backupz/32_messages_30_03_2025.txt"
    ...
    print(f"Duration of run_pipeline function: {total_time:.3f} seconds (From Main)")